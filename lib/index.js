...
export function AnimatedClusterFunc({ options, WrappedComponent, props, }) {
    ...
    useEffect(() => {
        ...
        const prev = prevMarkers.current
            .map((marker) => marker.id)
            .join('_');
        const next = props.markers.map((marker) => marker.id).join('_');
        if (prev !== next) {
            initialize();
        }
    });
     ...
     */
    function onRegionChanged(currentRegion) {
        const delta = currentRegion.longitudeDelta < 0
        ...
        const viewport = currentRegion.longitudeDelta >= 40
            ? { zoom: 1 }
            : GeoViewport.viewport(boundingBox, [options.width, options.height]);
        // @ts-ignore
        const nextClusters = superCluster.getClusters(boundingBox, viewport.zoom);
        ...
        animateMarkersIfNeeded(options.moveSpeed, originalMarkers, animatedMarkers, clusters, nextClusters);
     ...
     * Render the wrapped component with additional props.
     */
    return (React.createElement(WrappedComponent, { ...props, region: region, clusters: clusters, onRegionChanged: onRegionChanged, animatedMarkers: animatedMarkers }));
}
